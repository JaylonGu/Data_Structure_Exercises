...
List Find(List* L,int Addr,int N){
    for(int i=0;i<N;++i){
        if(L[i].Address==Addr) return L[i];
    }
}
...
int main(){
    int headptr,N,K;
    cin>>headptr>>N>>K;
    List *tmpL= new List[N];

    //Input
    for(int i=0;i<N;++i){
        scanf("%d%d%d",&tmpL[i].Address,&tmpL[i].Data,&tmpL[i].Next);
    }

    //Sort
    List *L=new List[N];
    L[0]=Find(tmpL,headptr,N);
    int nextptr=L[0].Next;
    int n=1;
    while(nextptr!=-1){
        L[n]=Find(tmpL,nextptr,N);
        nextptr=L[n++].Next;
    }
    delete [] tmpL;
